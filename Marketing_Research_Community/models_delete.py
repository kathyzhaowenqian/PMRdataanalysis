...
def get_compmany_default_value():
    return CompanyDelete.objects.get(id=6).company


class CommunityUserInfoDelete(UserInfo):
    class Meta:
        proxy =True
        managed=False
...
class CommunityProjectDetailDelete(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class CommunitySalesmanPositionDelete(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('CommunityUserInfoDelete', models.CASCADE, db_column='user',to_field='id') 
    company = models.ForeignKey('CompanyDelete', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class CommunityResearchListDelete(models.Model):
    salesmode_choices=[
        ('我司业务',
        (('直销', '直销'), ('代理', '代理'))
        ),
    ...
    salesman1 = models.ForeignKey('CommunityUserInfoDelete', models.CASCADE, db_column='salesman1',to_field='id',related_name='salesman1sqdelete',verbose_name= '第一负责人')
    salesman2 = models.ForeignKey('CommunityUserInfoDelete', models.CASCADE, db_column='salesman2',to_field='id',related_name='salesman2sqdelete',verbose_name= '第二负责人')
    ...
    operator = models.ForeignKey('CommunityUserInfoDelete', models.CASCADE, db_column='operator',to_field='id',related_name='operatorsqdelete',verbose_name= '最后操作人')
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        CommunityResearchDetailDelete.objects.filter(researchlist=self).update(is_active=False)
        CommunitySalesTargetDelete.objects.filter(researchlist=self).update(is_active=False)
        CommunityDetailCalculateDelete.objects.filter(researchlist=self).update(is_active=False)





class CommunityResearchDetailDelete(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('CommunityResearchListDelete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('CommunityProjectDetailDelete', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为CRP/SAA,则项目细分为CRP/SAA项目下的具体细分",null=True)
    ...
    competitionrelation = models.ForeignKey('CompetitionRelationDelete', models.CASCADE, db_column='competitionrelation',to_field='id',verbose_name= '竞品关系点',blank=True,null=True)
    createtime = models.DateTimeField(auto_now_add=True)
    updatetime = models.DateTimeField(auto_now=True)
    is_active=models.BooleanField(verbose_name='是否呈现',null=False, default = True)
...
class CommunitySalesTargetDelete(models.Model): 
...
class CommunityDetailCalculateDelete(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('CommunityResearchListDelete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

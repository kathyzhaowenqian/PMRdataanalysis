...
def get_compmany_default_value():
    return Company.objects.get(id=6).company


class CommunityUserInfo(UserInfo):
    class Meta:
        proxy =True
        managed=False
...
class CommunityProjectDetail(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class CommunitySalesmanPosition(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('CommunityUserInfo', models.CASCADE, db_column='user',to_field='id') 
    company = models.ForeignKey('Company', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class CommunityResearchList(models.Model):
    salesmode_choices=[
        ('我司业务',
        (('直销', '直销'), ('代理', '代理'))
        ),
    ...
    salesman1 = models.ForeignKey('CommunityUserInfo', models.CASCADE, db_column='salesman1',to_field='id',related_name='salesman1sq',verbose_name= '第一负责人')
    salesman2 = models.ForeignKey('CommunityUserInfo', models.CASCADE, db_column='salesman2',to_field='id',related_name='salesman2sq',verbose_name= '第二负责人')
    ...
    operator = models.ForeignKey('CommunityUserInfo', models.CASCADE, db_column='operator',to_field='id',related_name='operatorsq',verbose_name= '最后操作人')
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        CommunityResearchDetail.objects.filter(researchlist=self).update(is_active=False)
        CommunitySalesTarget.objects.filter(researchlist=self).update(is_active=False)
        CommunityDetailCalculate.objects.filter(researchlist=self).update(is_active=False)





class CommunityResearchDetail(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('CommunityResearchList', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('CommunityProjectDetail', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为CRP/SAA,则项目细分为CRP/SAA项目下的具体细分",null=True)
...
class CommunitySalesTarget(models.Model): 
...
class CommunityDetailCalculate(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('CommunityResearchList', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

...
class XEYgantt(View):
    ...
    def get(self, request):
        ...
        # if request.user.username=='admin':
            return render(request, 'XUERYUAN/gantt2.html')
        # else:
    ...
    def post(self,request):
        ...
        NewProjectStatus_queryset = XEYNewProjectStatus.objects.values('statushistory').filter(is_active=True)
        NegotiationStatus_queryset = XEYNegotiationStatus.objects.values('statushistory').filter(is_active=True)
        ChangeChannelStatus_queryset = XEYChangeChannelStatus.objects.values('statushistory').filter(is_active=True)
        ChangeBrandStatus_queryset = XEYChangeBrandStatus.objects.values('statushistory').filter(is_active=True)
        SetStatus_queryset = XEYSetStatus.objects.values('statushistory').filter(is_active=True)

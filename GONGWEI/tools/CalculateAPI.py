...
def GONGWEI(rawdata,filename):
    ...
    in_out_df=in_out_df[["订货抬头", "入库日期", "订单号", "科室", "商品编码", "商品名称", "规格", "单位", "品牌", "供应商", "采购单价", "税价总金额", "数量", "批号","生产日期", "有效期至", "备注", "备注2", "备注3"]]
...
#     #订单统计
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_order_calculate"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweiordercalculate_id_seq" RESTART WITH 1')
#     #订单明细(上传的)
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_order_detail"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweiorderdetail_id_seq" RESTART WITH 1')
#     #直送入库和康意路出入库明细(上传的)
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_in_out_detail"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweiinoutdetail_id_seq" RESTART WITH 1')
#     #康意路库存
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_kyl_stock"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweikylstock_id_seq" RESTART WITH 1')
#     #康意路库存(带批次)
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_kyl_stock_batch"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweikylstockbatch_id_seq" RESTART WITH 1')
#     #医院端的库存和领用汇总
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_hospital_in_out"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweihospitalinout_id_seq" RESTART WITH 1')
#     #医院端的库存和领用汇总(带批次)
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_hospital_in_out_batch"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweihospitalinoutbatch_id_seq" RESTART WITH 1')
#     #领用明细(上传的)
#     to_sql_connect_conn.execute('TRUNCATE TABLE "SUPPLYCHAIN"."GONGWEI_comsumption_detail"')   
#     to_sql_connect_conn.execute('ALTER SEQUENCE "SUPPLYCHAIN"."gongweiconsumptiondetail_id_seq" RESTART WITH 1')

#     try:
#         order_all_to_sql_df.to_sql("GONGWEI_order_calculate", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         order_to_sql_df.to_sql("GONGWEI_order_detail", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         in_out_to_sql_df.to_sql("GONGWEI_in_out_detail", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         str_kyl_to_sql_df.to_sql("GONGWEI_kyl_stock", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         product_batch_kyl_to_sql_df.to_sql("GONGWEI_kyl_stock_batch", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         product_in_consumption_to_sql_df.to_sql("GONGWEI_hospital_in_out", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         product_batch_in_consumption_to_sql_df.to_sql("GONGWEI_hospital_in_out_batch", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         consumption_to_sql_df.to_sql("GONGWEI_comsumption_detail", con=to_sql_connect, schema='SUPPLYCHAIN', if_exists='append', index=False)
#         print('已上传')
...
if __name__=='__main__':
    rawdata = 'C:\\Users\\赵文茜\\Desktop\\testgongwei.xlsx'  
    filename= 'C:\\Users\\赵文茜\\Desktop\\testgongwei_result.xlsx'  
    GONGWEI(rawdata,filename)
    print('原始数据已上传')

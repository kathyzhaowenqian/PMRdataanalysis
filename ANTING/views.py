...
class ATgantt(View):
    ...
    def get(self, request):
        ...
        # if request.user.username=='admin':
            return render(request, 'ANTING/gantt2.html')
        # else:
    ...
    def post(self,request):
        ...
        NewProjectStatus_queryset = ATNewProjectStatus.objects.values('statushistory').filter(is_active=True)
        NegotiationStatus_queryset = ATNegotiationStatus.objects.values('statushistory').filter(is_active=True)
        ChangeChannelStatus_queryset = ATChangeChannelStatus.objects.values('statushistory').filter(is_active=True)
        ChangeBrandStatus_queryset = ATChangeBrandStatus.objects.values('statushistory').filter(is_active=True)
        SetStatus_queryset = ATSetStatus.objects.values('statushistory').filter(is_active=True)

...
class IfTargetCustomerFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == '1':
            ...
            return queryset.filter(Q(detailcalculate__newold = '已有业务(含我司仪器)')| Q(detailcalculate__totalsumpermonth__gt = 0))
        elif self.value() == '3':
            return queryset.filter(Q(salestarget__is_active=True) & Q(salestarget__year='2023') & Q(salestarget__q1target = 0)& Q(salestarget__q2target =0) & Q(salestarget__q3target =0)& Q(salestarget__q4target=0)& Q(detailcalculate__newold = '新商机(不含我司仪器)') & Q(detailcalculate__totalsumpermonth = 0) )
...
class PMRResearchListAdmin(GlobalAdmin):
    ...
    ordering = ('-id',)#('-hospital__district','hospital__hospitalclass','hospital__hospitalname','salesman1','project',)#
    list_filter = [ProjectFilter,'hospital__district','hospital__hospitalclass',SalesmanFilter,SalesmanFilter2,'detailcalculate__newold',IfTargetCustomerFilter,IfActualSalesFilter]
...
#每季度实际完成额
    ...
    def actualsales_23_q1(self, obj):
        return obj.salestarget_set.filter(year='2023',is_active=True)[0].q1actualsales

    @admin.display(description='23/Q2实际')
    def actualsales_23_q2(self, obj):
        return obj.salestarget_set.filter(year='2023',is_active=True)[0].q2actualsales
    
    @admin.display(description='23/Q3实际')
    def actualsales_23_q3(self, obj):
        return obj.salestarget_set.filter(year='2023',is_active=True)[0].q3actualsales

    @admin.display(description='23/Q4实际')
    def actualsales_23_q4(self, obj):
        return obj.salestarget_set.filter(year='2023',is_active=True)[0].q4actualsales
    
    @admin.display(description='24/Q1实际')
    def actualsales_24_q1(self, obj):
        return obj.salestarget_set.filter(year='2024',is_active=True)[0].q1actualsales

    @admin.display(description='24/Q2实际')
    def actualsales_24_q2(self, obj):
        return obj.salestarget_set.filter(year='2024',is_active=True)[0].q2actualsales
    
    @admin.display(description='24/Q3实际')
    def actualsales_24_q3(self, obj):
        return obj.salestarget_set.filter(year='2024',is_active=True)[0].q3actualsales

    @admin.display(description='24/Q4实际')
    def actualsales_24_q4(self, obj):
        return obj.salestarget_set.filter(year='2024',is_active=True)[0].q4actualsales


#每季度实际完成率
    @admin.display(description='23/Q1完成率')
    def finishrate_23_q1(self, obj):
        if obj.salestarget_set.filter(year='2023',is_active=True)[0].q1target and obj.salestarget_set.filter(year='2023',is_active=True)[0].q1target != 0: #如果target不是0
            finishrate=obj.salestarget_set.filter(year='2023',is_active=True)[0].q1actualsales/obj.salestarget_set.filter(year='2023',is_active=True)[0].q1target
            obj.salestarget_set.filter(year='2023',is_active=True)[0].q1finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)

        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2023',is_active=True)[0].q1actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2023',is_active=True)[0].q1finishrate=finishrate
                return '--'
    ...
    def finishrate_23_q2(self, obj):
        if obj.salestarget_set.filter(year='2023',is_active=True)[0].q2target and obj.salestarget_set.filter(year='2023',is_active=True)[0].q2target != 0:
            finishrate=obj.salestarget_set.filter(year='2023',is_active=True)[0].q2actualsales/obj.salestarget_set.filter(year='2023',is_active=True)[0].q2target
            obj.salestarget_set.filter(year='2023',is_active=True)[0].q2finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)

        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2023',is_active=True)[0].q2actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2023',is_active=True)[0].q2finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_23_q3(self, obj):
        if obj.salestarget_set.filter(year='2023',is_active=True)[0].q3target and obj.salestarget_set.filter(year='2023',is_active=True)[0].q3target != 0:
            finishrate=obj.salestarget_set.filter(year='2023',is_active=True)[0].q3actualsales/obj.salestarget_set.filter(year='2023',is_active=True)[0].q3target
            obj.salestarget_set.filter(year='2023',is_active=True)[0].q3finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)

        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2023',is_active=True)[0].q3actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2023',is_active=True)[0].q3finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_23_q4(self, obj):
        if obj.salestarget_set.filter(year='2023',is_active=True)[0].q4target and obj.salestarget_set.filter(year='2023',is_active=True)[0].q4target != 0:
            finishrate=obj.salestarget_set.filter(year='2023',is_active=True)[0].q4actualsales/obj.salestarget_set.filter(year='2023',is_active=True)[0].q4target
            obj.salestarget_set.filter(year='2023',is_active=True)[0].q4finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)

        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2023',is_active=True)[0].q4actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2023',is_active=True)[0].q4finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_24_q1(self, obj):
        if obj.salestarget_set.filter(year='2024',is_active=True)[0].q1target and obj.salestarget_set.filter(year='2024',is_active=True)[0].q1target != 0:
            finishrate=obj.salestarget_set.filter(year='2024',is_active=True)[0].q1actualsales/obj.salestarget_set.filter(year='2024',is_active=True)[0].q1target
            obj.salestarget_set.filter(year='2024',is_active=True)[0].q1finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)

        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2024',is_active=True)[0].q1actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2024',is_active=True)[0].q1finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_24_q2(self, obj):
        if obj.salestarget_set.filter(year='2024',is_active=True)[0].q2target and obj.salestarget_set.filter(year='2024',is_active=True)[0].q2target != 0:
            finishrate=obj.salestarget_set.filter(year='2024',is_active=True)[0].q2actualsales/obj.salestarget_set.filter(year='2024',is_active=True)[0].q2target
            obj.salestarget_set.filter(year='2024',is_active=True)[0].q2finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)
        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2024',is_active=True)[0].q2actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2024',is_active=True)[0].q2finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_24_q3(self, obj):
        if obj.salestarget_set.filter(year='2024',is_active=True)[0].q3target and obj.salestarget_set.filter(year='2024',is_active=True)[0].q3target != 0:
            finishrate=obj.salestarget_set.filter(year='2024',is_active=True)[0].q3actualsales/obj.salestarget_set.filter(year='2024',is_active=True)[0].q3target
            obj.salestarget_set.filter(year='2024',is_active=True)[0].q3finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)
        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2024',is_active=True)[0].q3actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2024',is_active=True)[0].q3finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
    ...
    def finishrate_24_q4(self, obj):
        if obj.salestarget_set.filter(year='2024',is_active=True)[0].q4target and obj.salestarget_set.filter(year='2024',is_active=True)[0].q4target != 0:
            finishrate=obj.salestarget_set.filter(year='2024',is_active=True)[0].q4actualsales/obj.salestarget_set.filter(year='2024',is_active=True)[0].q4target
            obj.salestarget_set.filter(year='2024',is_active=True)[0].q4finishrate=finishrate
            return '{:.1f}%'.format(finishrate*100)
        else: #如果target是0 但actual不是0
            if obj.salestarget_set.filter(year='2024',is_active=True)[0].q4actualsales !=0:  
                finishrate=0
                obj.salestarget_set.filter(year='2024',is_active=True)[0].q4finishrate=finishrate
                return '--'
            else: #如果target是0 actual是0
...
class PMRResearchDetailAdmin(GlobalAdmin):
    ...
    list_filter = ['researchlist__hospital__district','researchlist__hospital__hospitalclass',ProjectFilter,SalesmanFilterforDetail,'competitionrelation','researchlist__detailcalculate__newold','expiration']

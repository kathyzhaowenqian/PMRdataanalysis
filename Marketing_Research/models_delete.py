...
def get_compmany_default_value():
    return CompanyDelete.objects.get(id=1).company


class UserInfoDelete(UserInfo):
...
class ProjectDetailDelete(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class SalesmanPositionDelete(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('UserInfoDelete', models.CASCADE, db_column='user',to_field='id') #settings.AUTH_USER_MODEL
    company = models.ForeignKey('CompanyDelete', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class PMRResearchListDelete(models.Model):
    ...
    salesman1 = models.ForeignKey('UserInfoDelete', models.CASCADE, db_column='salesman1',to_field='id',related_name='salesman1delete',verbose_name= '第一负责人')
    salesman2 = models.ForeignKey('UserInfoDelete', models.CASCADE, db_column='salesman2',to_field='id',related_name='salesman2delete',verbose_name= '第二负责人')
    ...
    operator = models.ForeignKey('UserInfoDelete', models.CASCADE, db_column='operator',to_field='id',related_name='operatordelete',verbose_name= '最后操作人')
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        self.save()
        #可以把相关联得 Objects 也一起伪删除
        PMRResearchDetailDelete.objects.filter(researchlist=self).update(is_active=False)
        SalesTargetDelete.objects.filter(researchlist=self).update(is_active=False)
        DetailCalculateDelete.objects.filter(researchlist=self).update(is_active=False)



class PMRResearchDetailDelete(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('PMRResearchListDelete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('ProjectDetailDelete', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为WAKO,则项目细分为WAKO项目下的具体细分",null=True)
...
class SalesTargetDelete(models.Model): 
...
class DetailCalculateDelete(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('PMRResearchListDelete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

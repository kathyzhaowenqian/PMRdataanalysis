...
def get_compmany_default_value():
    return CompanyDelete.objects.get(id=3).company


class UserInfo2Delete(UserInfo):
...
class ProjectDetail2Delete(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class SalesmanPosition2Delete(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('UserInfo2Delete', models.CASCADE, db_column='user',to_field='id') #settings.AUTH_USER_MODEL
    company = models.ForeignKey('CompanyDelete', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class PMRResearchList2Delete(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        PMRResearchDetail2Delete.objects.filter(researchlist=self).update(is_active=False)
        SalesTarget2Delete.objects.filter(researchlist=self).update(is_active=False)
        DetailCalculate2Delete.objects.filter(researchlist=self).update(is_active=False)



class PMRResearchDetail2Delete(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('PMRResearchList2Delete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('ProjectDetail2Delete', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为WAKO,则项目细分为WAKO项目下的具体细分",null=True)
...
class SalesTarget2Delete(models.Model): 
...
class DetailCalculate2Delete(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('PMRResearchList2Delete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

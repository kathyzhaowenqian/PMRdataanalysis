...
def get_compmany_default_value():
    return Company.objects.get(id=3).company
...
class ProjectDetail2(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class SalesmanPosition2(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('UserInfo2', models.CASCADE, db_column='user',to_field='id') #settings.AUTH_USER_MODEL
    company = models.ForeignKey('Company', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class PMRResearchList2(models.Model):
    salesmode_choices=[
        ('我司业务',
        (('直销', '直销'), ('代理', '代理'))
        ),
    ...
    salesman1 = models.ForeignKey('UserInfo2', models.CASCADE, db_column='salesman1',to_field='id',related_name='salesman1wd',verbose_name= '第一负责人')
    salesman2 = models.ForeignKey('UserInfo2', models.CASCADE, db_column='salesman2',to_field='id',related_name='salesman2wd',verbose_name= '第二负责人')
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        PMRResearchDetail2.objects.filter(researchlist=self).update(is_active=False)
        SalesTarget2.objects.filter(researchlist=self).update(is_active=False)
        DetailCalculate2.objects.filter(researchlist=self).update(is_active=False)





class PMRResearchDetail2(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('PMRResearchList2', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('ProjectDetail2', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为真菌,则项目细分为真菌项目下的具体细分",null=True)
...
class SalesTarget2(models.Model): 
...
class DetailCalculate2(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('PMRResearchList2', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

...
import xlwt
import io
from SHIYINAN.tools.CalculateAPI import *
import os
from django.http import FileResponse
today = date.today()
folder_name1 = 'file'
folder_name2 = 'syn'
file_name = 'ShiYiNan.xlsx'
...
class Upload2(View):
    def get(self,request):
        if request.user.username=='admin' or  request.user.username=='zwq8zhj' or  request.user.username=='cxy':               
            ...
            return render(request,'SYN/index.html')
        else:
    ...
    def post(self,request):
        ...
        #     # 使用 Pandas 读取 Excel 文件
            try:
                ...
                SHIYINAN(uploaded_file,file_name)
...
class Download2(View):
    def get(self,request):
        print('走到Download2 get')
        # login_user = request.user.chinesename
        if request.user.username=='admin' or  request.user.username=='zwq8zhj' or  request.user.username=='cxy':  
            file_path = Upload_File_2

            print('Download2 file_path:::',file_path)
            ...
            try:
                with open(file_path, 'rb') as f:
                    ...
                    response['Content-Disposition'] = 'attachment; filename=ShiYiNanCalculate.xlsx'

...
def get_compmany_default_value():
    return CompanyDelete.objects.get(id=2).company


class UserInfo3Delete(UserInfo):
...
class ProjectDetail3Delete(models.Model):
    # id = models.BigAutoField(primary_key=True)
...
class SalesmanPosition3Delete(models.Model):
    # id = models.BigAutoField(primary_key=True)
    user = models.ForeignKey('UserInfo3Delete', models.CASCADE, db_column='user',to_field='id') #settings.AUTH_USER_MODEL
    company = models.ForeignKey('CompanyDelete', models.CASCADE, db_column='company',to_field='id',verbose_name= '公司',default=get_compmany_default_value)
...
class PMRResearchList3Delete(models.Model):
    ...
    salesman1 = models.ForeignKey('UserInfo3Delete', models.CASCADE, db_column='salesman1',to_field='id',related_name='salesman1qtdelete',verbose_name= '第一负责人')
    salesman2 = models.ForeignKey('UserInfo3Delete', models.CASCADE, db_column='salesman2',to_field='id',related_name='salesman2qtdelete',verbose_name= '第二负责人')
    ...
    progress = models.TextField(verbose_name='进展',max_length=255, blank=True, null=True,choices=progress_choices,help_text=u"仅填报目标新项目的进展状态")

    adminmemo=models.TextField(verbose_name='备注',max_length=500, blank=True, null=True)
    operator = models.ForeignKey('UserInfo3Delete', models.CASCADE, db_column='operator',to_field='id',related_name='operatorqtdelete',verbose_name= '最后操作人')
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        PMRResearchDetail3Delete.objects.filter(researchlist=self).update(is_active=False)
        SalesTarget3Delete.objects.filter(researchlist=self).update(is_active=False)
        DetailCalculate3Delete.objects.filter(researchlist=self).update(is_active=False)



class PMRResearchDetail3Delete(models.Model):
    ownbusiness_choices = (
        (True, '是'),
        (False, '否'),)
    # id = models.BigAutoField(primary_key=True)
    researchlist = models.ForeignKey('PMRResearchList3Delete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')
    #是否要关联detailproject表？？？
    detailedproject = models.ForeignKey('ProjectDetail3Delete', models.CASCADE, db_column='detailedproject',to_field='id',verbose_name= '项目细分(注意根据本页主项目填报)',help_text=u"例如:本页项目为WAKO,则项目细分为WAKO项目下的具体细分",null=True)
...
class SalesTarget3Delete(models.Model): 
...
class DetailCalculate3Delete(models.Model):
    id = models.BigAutoField(primary_key=True)
    researchlist = models.OneToOneField('PMRResearchList3Delete', models.CASCADE, db_column='researchlist',to_field='id',verbose_name= '调研列表')

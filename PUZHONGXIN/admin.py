...
class PZXSPDListAdmin(GlobalAdmin):
    ...
    empty_value_display = '--'
    list_per_page = 10
    list_display = ('salesman_chinesename','supplier','brand','department','product','machinemodel','listotal','salestotal','display_salestotalpercent','purchasetotal','display_gppercent','relation',
                    )
    ordering = ('id',)
...
# ------delete_model内层的红色删除键------------------------------
    ...
    def get_queryset(self, request):
        ...
        print('PZXSPDListAdmin-get_queryset')
        #通过外键连list中的负责人名称
        if request.user.is_superuser  or request.user.groups.values()[0]['name'] =='boss' or request.user.groups.values()[0]['name'] =='pzx' or request.user.groups.values()[0]['name'] =='JConlyview' or request.user.groups.values()[0]['name'] =='allviewonly': 

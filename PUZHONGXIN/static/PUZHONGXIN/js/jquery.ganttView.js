...
(function (jQuery) {
	...
	var Chart = function(div, opts) {
		...
		function addHzHeader(div, dates, cellWidth) {
			...
			var weekDiv = jQuery("<div>", { "class": "ganttview-hzheader-week" });
			var totalW = 0;
			for (var y in dates) {
				for (var m in dates[y]) {
					...
					for (var d in dates[y][m]) {
						var style = addStyle(1, dates[y][m][d]);
						daysDiv.append(jQuery("<div>", { "class": style }).append(dates[y][m][d].getDate()));
						weekDiv.append(jQuery("<div>", { "class": style }).append(opts.dowNames[dates[y][m][d].getDay()]));
					}
				}
			}
			monthsDiv.css("width", totalW + "px");
			daysDiv.css("width", totalW + "px");
			weekDiv.css("width", totalW + "px");
			headerDiv.append(monthsDiv).append(daysDiv);
			headerDiv.append(daysDiv).append(weekDiv);
			div.append(headerDiv);
		}

		function addStyle(type, date) {
			var ret = "";
			if (type==1)
				ret = "ganttview-hzheader-day";
			else if (type==2)
				ret = "ganttview-grid-row-cell";

			ret += DateUtils.isWeekend(date) ? " ganttview-weekend ":"";
			ret += DateUtils.isToday(date) ? " ganttview-istoday ":"";
		...
		function addGrid(div, data, dates, cellWidth, showWeekends) {
			...
			for (var y in dates) {
				for (var m in dates[y]) {
					for (var d in dates[y][m]) {
						var style = addStyle(2, dates[y][m][d]);
						var cellDiv = jQuery("<div>", { "class": style });
			...
			for (var i = 0; i < data.length; i++) {
				for (var j = 0; j < data[i].series.length; j++) {
					...
					var block = jQuery("<div>", {
						...
						"title": series.name + ", " + size + " " + opts.confNames[0],
						"css": {
					...
					block.append(jQuery("<div>", { "class": "ganttview-block-text" }).text(size + opts.confNames[0]));
	...
	var Behavior = function (div, opts) {
		...
		function updateDataAndPosition(div, block, cellWidth, startDate) {
			...
			jQuery("div.ganttview-block-text", block).text((numberOfDays + 1) + opts.confNames[0] );
